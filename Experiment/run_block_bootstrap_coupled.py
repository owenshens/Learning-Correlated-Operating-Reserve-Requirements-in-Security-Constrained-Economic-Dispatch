"""
Block bootstrap confidence intervals for test coverage (coupled experiment).

Loads saved models from run_coupled_experiment.py to ensure consistency.
For Box and SampleCov, coverage indicators are the same as decoupled
(L and scores don't change with the dispatch formulation).
...
def main():
    print("=" * 60)
    print("Block Bootstrap CIs for Test Coverage (Coupled)")
    print("=" * 60)
    ...
    # Box and SampleCov: L is the same as decoupled, so coverage indicators are identical.
    # The coupled dispatch changes cost but not the gauge scores or coverage.
    ...
    # Method 3: Static L (load saved coupled model)
    print("Static L method (loading saved coupled model)...")
    L_static = np.load(os.path.join(models_dir, "L_static_coupled.npy"))
    ...
    # Method 4: NN (load saved coupled model)
    print("NN method (loading saved coupled model)...")
    model = ContextNN(d_context=7, d_uncertainty=d, hidden_dims=[128, 64])
    model.load_state_dict(torch.load(os.path.join(models_dir, "nn_model_coupled.pt"), weights_only=True))
    model.eval()
    ...
    # --- Compute CIs ---
    results = {"block_length": block_length, "n_bootstrap": n_bootstrap}

    for name, info in methods_info.items():
        indicators = info["indicators"]
        cov = info["coverage"]
        wilson_lo, wilson_hi = wilson_ci(cov, len(u_test))
        block_lo, block_hi = block_bootstrap_ci(indicators, block_length, n_bootstrap)

        results[name] = {
            "test_coverage": cov,
            "wilson_ci": [float(wilson_lo), float(wilson_hi)],
            "block_ci": [float(block_lo), float(block_hi)],
        }

    # --- Print summary ---
    print("\n" + "=" * 60)
    print("SUMMARY (Coupled Experiment)")
    print("=" * 60)
    print(f"\nBlock length: {block_length} hours, Bootstrap replicates: {n_bootstrap}")
    print(f"\n{'Method':<16} {'Coverage':<10} {'Wilson CI':<24} {'Block CI':<24}")
    print("-" * 74)
    ...
    # Save
    output_path = os.path.join(DATA_DIR, "block_bootstrap_coupled_ci.json")
